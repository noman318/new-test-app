...
const updateDetailPackage = async (req, res, next) => {
    ...
    try {
        ...
        const detailToUpdate = packageData.details.find(detail => detail._id.toString() === detail_id);
        if (!detailToUpdate) {
...
const createDetailPackageItems = async (req, res, next) => {
    try {
        ...
        if (packageData) {
            ...
            if (!Array.isArray(packageData.details) || packageData.details.length === 0) {
...
const createPackageItems = async (req, res, next) => {
    try {
        const { price, title, category, description,pickUp_drop,duration,inclusions,itinerary } = req.body;
